...
export type PostTrialMotation = MotionBase & {
  classification: 'acquittal' | 'correction' | 'new trial'
  step: 'post-trial'
}

export type TrialMotation = PreTrialMotion | PostTrialMotation

export type AllowedMotion = TrialMotation & {
  status: 'allowed'
  deliberationHours: number
}

export type DeniedMotion = TrialMotation & {
  status: 'denied'
  deliberationHours: number
  annoyedJustice: boolean
}

export type PendingMotion = TrialMotation & {
  status: 'pending'
